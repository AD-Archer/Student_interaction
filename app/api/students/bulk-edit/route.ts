...
export async function POST(request: NextRequest) {
  try {
    ...
    if (/^\d+$/.test(startId) && /^\d+$/.test(endId)) {
      ...
      // Only use 4-digit numbers for IDs
      function normalizeStudentId(id: string) {
        return id.replace(/[^\d]/g, "").padStart(4, '0');
      }
      const startStudentId = normalizeStudentId(startId);
      const endStudentId = normalizeStudentId(endId);
      if (!startStudentId || !endStudentId) {
        inRange = [];
      } else {
        const min = startStudentId < endStudentId ? startStudentId : endStudentId;
        const max = startStudentId > endStudentId ? startStudentId : endStudentId;
